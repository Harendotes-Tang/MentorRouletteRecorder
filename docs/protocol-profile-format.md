...
- `.json.contradicted`（§11.4，流量证伪后自动撤下的）**永不可恢复**：不是玩家选择停用的，
  明天它读到的仍是同一条错误报文；
- 当前生效的档案（含正在使用的共享档案）先按停止捕获收尾并解绑，重新读目录后由于
  目录优先级为 随包 > 本机 > 共享，恢复出来的本机档案会被选中；
...
- 恢复出来的文件与其他档案一样要过 `ProfileLoader` 校验。万一过不了（由更早版本写出、
  或已损坏），它会被重新改回 `.retired`，避免此后每次选择都在拒绝同一个文件，
...
因为这类档案下校准本来就一直开着。读得出服务器匹配报文的档案则不然：校准已经结束，
...
- 确认后按同一 `profile_id` 重写档案，会话内若没有记录在进行中即刻换用新解析器；角色随即重新求值，
  档案有了职业便不再处于这一角色。
