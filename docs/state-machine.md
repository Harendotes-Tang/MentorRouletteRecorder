...
**再次弹出也要让桌面端知道。** 上述两种情况下状态在前后都是 `MENTOR_MATCHED`，而状态事件原本只在状态变化时发出，
于是第二次弹窗没有任何事件，也就没有语音。状态机为此记下「这是本条记录的第几次匹配」（`MatchOffers`）：
与上一次弹窗相隔 10 秒及以上（`NewOfferGap`）的再次弹出加一，同一次匹配连发的数条报文相隔不到 1 秒，不加。
宿主在该数值增加、或同一状态下记录换了一条时，再发一次 `run_state_changed`，载荷带 `match_offer`
（见 `contracts/CHANGELOG.md` 2026-09-20）。
...
| 值 | 条件 |
|---|---|
| `HIGH` | 匹配、进入、结束三个关键事件全部为 verified，结局由结算报文给出（而非由离开副本推断），副本已由报文认出（`content_id` 或 `territory_id` 至少其一来自协议事件），且 `job_id` 解析成功 |
| `MEDIUM` | 关键事件齐全但存在字段缺失（副本未能由报文认出，或 `job_id` 为 NULL），或结局由离开副本推断得出 |
| `LOW` | 关键事件缺失（重启恢复、队列溢出、连接异常收尾） |
| `NONE` | 手工创建的记录，或 `UNKNOWN_FINAL_STATE` |

国服客户端以区域编号指明副本，不发送 `content_id`；1.4.0 之前 `HIGH` 只认 `content_id`，以区域编号认出副本的记录因此无法达到 `HIGH`。
档案不含结算报文时（§3.10），记录以 `UNKNOWN` 结束并评为 `LOW`，不受本条影响。
`territory_id` 在状态机内只来自协议事件（进入副本的报文或区域报文），§3 的本地展示映射不会写入它，因此上述放宽不违背"展示映射不提高置信度"。
置信度在记录**结束时**评定；进行中的记录存有初始值 `MEDIUM`，界面显示为「结束时评定」而非「中」。
