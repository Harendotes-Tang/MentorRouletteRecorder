...
| 值 | 条件 |
|---|---|
| `HIGH` | 匹配、进入、结束三个关键事件全部为 verified，副本已由报文认出（`content_id` 或 `territory_id` 至少其一来自协议事件），且 `job_id` 解析成功 |
| `MEDIUM` | 关键事件齐全但存在字段缺失（副本未能由报文认出，或 `job_id` 为 NULL） |
| `LOW` | 关键事件缺失（重启恢复、队列溢出、连接异常收尾） |
| `NONE` | 手工创建的记录，或 `UNKNOWN_FINAL_STATE` |

国服客户端以区域编号指明副本，不发送 `content_id`；1.4.0 之前 `HIGH` 只认 `content_id`，国服的记录因此无法达到 `HIGH`。
